...
function ErrorModal(props:IError):React.ReactElement {
    const { onChangeErr, title } = props;


    return (
        <div className="w-full  h-full bg-opacity-20 bg-black  fixed top-0 left-0 flex items-center justify-center" onClick={onChangeErr}>
            <div
                className="flex flex-1 w-100 gap-4 flex-col  bg text-white bg-slate-900 py-6 px-8 rounded-md shadow-lg md:w-1/2"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="flex justify-end">
                    <button onClick={onChangeErr} className="rounded-full overflow-hidden hover:scale-110">
                        {CloseIcon}
                    </button>
                </div>
